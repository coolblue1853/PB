...
namespace MoreMountains.CorgiEngine
{
	...
	public class SkillManager_Z : CharacterAbility
	{
		...
		{
			// 
			if ((_inputManager.Skill1Button.State.CurrentState == MMInput.ButtonStates.ButtonDown) || (_inputManager.ShootAxis == MMInput.ButtonStates.ButtonDown))
			{
				canIntrupted = false;
				ShootStart();
			}
			...
			if (CurrentWeapon != null)
			{
				if (ContinuousPress && (CurrentWeapon.TriggerMode == Weapon.TriggerModes.Auto) && (_inputManager.Skill1Button.State.CurrentState == MMInput.ButtonStates.ButtonDown))
				{
					ShootStart();
				}
			...
			if ((_inputManager.Skill1Button.State.CurrentState == MMInput.ButtonStates.ButtonUp) || (_inputManager.ShootAxis == MMInput.ButtonStates.ButtonUp))
			{
				ShootStop();
			}
			...
			if (CurrentWeapon != null)
			{
				if ((CurrentWeapon.WeaponState.CurrentState == Weapon.WeaponStates.WeaponDelayBetweenUses)
					&& ((_inputManager.ShootAxis == MMInput.ButtonStates.Off) && (_inputManager.Skill1Button.State.CurrentState == MMInput.ButtonStates.Off)))
				{
